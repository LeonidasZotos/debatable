...
model = SentenceTransformer(model_name, device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
...
# Setup Dev set
evaluator = BinaryClassificationEvaluator.from_input_examples(dev, name = 'mrpc-dev')
...
model = SentenceTransformer(model_save_path, device = torch.device('cuda:0' if torch.cuda.is_available() else 'cpu'))
test_evaluator = BinaryClassificationEvaluator.from_input_examples(test, name = 'mrpc-test')
